...
export function CategoryListingPage() {
  ...
  const FilterSidebarContent = () => (
    <div className="flex flex-col gap-6">
      {/* Categories */}
      <div>
        <h3 className="text-sm font-bold text-[#45055B] mb-3 uppercase tracking-wider">Categories</h3>
        <ul className="space-y-1">
          <li>
            <button 
              onClick={() => handleCategoryChange('all')}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${categoryId === 'all' ? 'bg-[#45055B]/10 text-[#D4AF37] font-bold' : 'text-[#45055B]/70 hover:bg-gray-100'}`}
            >
              All Products
            </button>
          </li>
          {categories.map(cat => (
            <li key={cat.id}>
              <button 
                ...
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${categoryId === cat.id.toString() ? 'bg-[#45055B]/10 text-[#D4AF37] font-bold' : 'text-[#45055B]/70 hover:bg-gray-100'}`}
              >
                {cat.name}
              </button>
            </li>
      ...
      {currentModels.length > 0 && (
        <div className="border-t border-[#D4AF37]/20 pt-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-[#45055B] uppercase tracking-wider">Subcategories</h3>
            {modelQuery && (
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
            {currentModels.map(model => (
              <label key={model} className="flex items-center gap-2 cursor-pointer group">
                <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${modelQuery === model ? 'border-[#D4AF37] bg-[#45055B]' : 'border-gray-300 group-hover:border-[#D4AF37]'}`}>
                  {modelQuery === model && <Check className="w-3 h-3 text-white" />}
                </div>
                <span className={`text-sm ${modelQuery === model ? 'text-[#D4AF37] font-bold' : 'text-[#45055B]/70'}`}>{model}</span>
      ...
      <div className="border-t border-[#D4AF37]/20 pt-6">
        <h3 className="text-sm font-bold text-[#45055B] mb-3 uppercase tracking-wider">Offers</h3>
        <label className="flex items-center gap-3 cursor-pointer group select-none">
          <div
            ...
            className={`w-10 h-5 rounded-full relative transition-colors duration-200 flex-shrink-0 ${
              showOnlyOffers ? 'bg-[#45055B]' : 'bg-gray-200'
            }`}
          >
          ...
          <div className="flex flex-col">
            <span className={`text-sm font-semibold ${
              showOnlyOffers ? 'text-[#D4AF37]' : 'text-[#45055B]/70'
            }`}>Reduced Price</span>
            <span className="text-[10px] text-[#45055B]/40">Show only discounted items</span>
          </div>
        </label>
      </div>
      ...
      {/* Sort By */}
      <div className="border-t border-[#D4AF37]/20 pt-6">
        <h3 className="text-sm font-bold text-[#45055B] mb-3 uppercase tracking-wider">Sort By</h3>
        <div className="space-y-2">
          {[
            ...
            <label key={opt.id} className="flex items-center gap-2 cursor-pointer group">
              <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${sortBy === opt.id ? 'border-[#D4AF37] bg-[#45055B]' : 'border-gray-300 group-hover:border-[#D4AF37]'}`}>
                {sortBy === opt.id && <Check className="w-3 h-3 text-white" />}
              </div>
              <span className={`text-sm ${sortBy === opt.id ? 'text-[#D4AF37] font-bold' : 'text-[#45055B]/70'}`}>{opt.label}</span>
  ...
  return (
    <div className="bg-[#FAF6F0] min-h-screen pb-20">
      ...
      {/* Category Banner */}
      <div className="reveal-on-scroll w-full bg-[#45055B]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
        ...
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1">
            <div className={layout === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 sm:gap-6 lg:gap-8 max-w-lg sm:max-w-none mx-auto' : 'flex flex-col gap-6'}>
        ...
        <div className="fixed inset-0 z-[60] lg:hidden flex">
          ...
          <div className="relative ml-auto w-[85%] max-w-sm bg-white h-full flex flex-col shadow-2xl transition-transform transform translate-x-0">
            <div className="flex items-center justify-between p-5 border-b border-[#D4AF37]/20">
              <h2 className="font-serif text-xl font-bold text-[#45055B] flex items-center gap-2">
            ...
            <div className="p-5 border-t border-[#D4AF37]/20 bg-gray-50 flex gap-3">
              <button 
                ...
                className="flex-1 px-4 py-3 border border-[#D4AF37]/20 text-[#45055B] font-bold rounded-xl bg-white shadow-sm"
              >
                Reset
              </button>
              <button 
                onClick={() => setShowMobileFilters(false)}
                className="flex-[2] px-4 py-3 bg-[#45055B] text-white font-bold rounded-xl shadow-md"
              >
                Apply Filters
              </button>
            </div>
