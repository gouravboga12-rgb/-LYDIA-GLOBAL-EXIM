          ...
          <div className="flex flex-wrap gap-2">
            {['cash', 'upi', 'bank_transfer', 'stripe', 'other'].map(m => (
              <button key={m} onClick={() => setMarkMethod(m)}
                className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors capitalize ${markMethod === m ? 'bg-[#2A0845] text-white border-[#2A0845]' : 'bg-white text-[#2A0845]/60 border-[#2A0845]/20 hover:border-[#2A0845]/40'
                  }`}>{m.replace('_', ' ')}</button>
            ))}
          </div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        {result ? (
          ...
          <div className="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            {/* Shipping Address */}
            <div>
              <p className="text-[10px] font-bold text-[#2A0845]/40 uppercase tracking-wider mb-3">Shipping Address</p>
              <div className="grid grid-cols-2 gap-2">
                {[['name', 'Full Name'], ['line1', 'Address Line 1'], ['line2', 'Line 2 (optional)'], ['city', 'City'], ['state', 'State'], ['pincode', 'ZIP']].map(([field, label]) => (
                  <div key={field} className={field === 'line1' ? 'col-span-2' : ''}>
                    <label className="text-[10px] font-bold text-[#2A0845]/50 uppercase tracking-wider block mb-1">{label}</label>
                    {field === 'line1' ? (
                      <AddressAutocomplete
                        value={address[field] || ''}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select value={countryCode} onChange={e => setCountryCode(e.target.value)}
                  className="w-24 text-sm border border-[#2A0845]/15 rounded-xl px-2 py-2 bg-[#FAF6F0] text-[#2A0845] focus:outline-none focus:border-[#2A0845]/30">
                ...
                <input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)}
                  ...
                  className="flex-1 text-sm border border-[#2A0845]/15 rounded-xl px-3 py-2 bg-[#FAF6F0] text-[#2A0845] focus:outline-none focus:border-[#2A0845]/30" />
              </div>
            </div>

            {/* Manual Tracking */}
            <div>
              <p className="text-[10px] font-bold text-[#2A0845]/40 uppercase tracking-wider mb-2">Manual Tracking (For alternative partners)</p>
              <div className="grid grid-cols-2 gap-2">
            ...
            {(originalTax > 0 || originalDiscount > 0) && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 space-y-2">
                <p className="text-[10px] font-bold text-[#2A0845]/60 uppercase tracking-wider mb-1">Price Adjustments</p>
                {originalTax > 0 && (
                  <label className="flex items-center gap-2 text-sm text-[#2A0845] cursor-pointer font-medium">
                    <input type="checkbox" checked={applyProratedTax} onChange={e => setApplyProratedTax(e.target.checked)} className="w-4 h-4 rounded text-[#2A0845] focus:ring-[#2A0845]" />
                    Prorate Tax calculation based on new item total
                  </label>
                )}
                {originalDiscount > 0 && (
                  <label className="flex items-center gap-2 text-sm text-[#2A0845] cursor-pointer font-medium">
                    <input type="checkbox" checked={applyProratedDiscount} onChange={e => setApplyProratedDiscount(e.target.checked)} className="w-4 h-4 rounded text-[#2A0845] focus:ring-[#2A0845]" />
                ...
                <div className="space-y-2">
                  {remainingShipping > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#2A0845]/10 cursor-pointer">
                      ...
                      <span className="font-bold text-[#2A0845]">${remainingShipping.toFixed(2)}</span>
                    </label>
                  )}
                  {remainingTax > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#2A0845]/10 cursor-pointer">
                      ...
                      <span className="font-bold text-[#2A0845]">${actualTaxRefund.toFixed(2)}</span>
                    </label>
                  )}
                  {remainingSignature > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#2A0845]/10 cursor-pointer">
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={refundSignature} onChange={e => setRefundSignature(e.target.checked)} className="w-4 h-4 accent-[#2A0845]" />
                        <span className="text-sm font-bold text-[#2A0845]">Refund Signature Fee</span>
                      </div>
                      <span className="font-bold text-[#2A0845]">${actualSignatureRefund.toFixed(2)}</span>
                    </label>
                  )}
                  {remainingInsurance > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#2A0845]/10 cursor-pointer">
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={refundInsurance} onChange={e => setRefundInsurance(e.target.checked)} className="w-4 h-4 accent-[#2A0845]" />
                        <span className="text-sm font-bold text-[#2A0845]">Refund Insurance Fee</span>
                      </div>
                      <span className="font-bold text-[#2A0845]">${actualInsuranceRefund.toFixed(2)}</span>
                    </label>
                  )}
                  {remainingDiscount > 0 && (
              ...
              {expanded === order.id && (
                <div className="border-t border-[#2A0845]/5 p-3 sm:p-4 lg:p-5 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-sans text-[#2A0845]/40 uppercase tracking-wider mb-2">Shipment</p>
                      {order.tracking_id && order.tracking_id.trim() !== "" ? (
                          ...
                          <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2">
                            ...
                            {order.tracking_link && (
                              <a href={order.tracking_link} target="_blank" rel="noopener noreferrer"
                                className="ml-auto flex-shrink-0 text-[#2A0845] hover:opacity-80 transition-colors">
                                <ExternalLink className="w-3.5 h-3.5" />
                              </a>
                            )}
                          </div>
                          {order.shipping_label_url && (
                            <a href={order.shipping_label_url} target="_blank" rel="noopener noreferrer" className="mt-2 w-full flex items-center justify-center gap-2 bg-[#2A0845] text-white px-4 py-2 rounded-xl text-xs font-semibold hover:opacity-90">
                              📄 Download Label
                            </a>
                          )}
                          <div className="flex justify-between mt-2">
                            <button onClick={() => setShippoConfigModal(order)} disabled={shipping[`shippo_${order.id}`] || order.status === 'cancelled'}
                              className="text-[10px] font-sans text-[#2A0845]/50 hover:text-[#2A0845] transition-colors underline w-full text-center disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <div className="space-y-2">
                          <button onClick={() => setShippoConfigModal(order)} disabled={shipping[`shippo_${order.id}`] || order.status === 'cancelled'}
                            className="w-full flex items-center justify-center gap-2 bg-[#D4AF37]/20 hover:bg-[#D4AF37]/30 text-[#2A0845] px-4 py-2 rounded-xl text-xs font-semibold font-sans transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                      ...
                      return (
                        <div className="bg-[#FAF6F0] rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          {address.signature_required && (
                            <div className="flex items-start gap-2">
                              <span className="text-[10px] font-bold text-[#2A0845]/40 uppercase tracking-wider w-14 shrink-0 mt-0.5">Extras</span>
