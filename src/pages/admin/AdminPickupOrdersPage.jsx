            ...
            {['cash', 'upi', 'bank_transfer', 'stripe', 'other'].map(m => (
              <button key={m} onClick={() => setMarkMethod(m)}
                className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors capitalize ${
                  markMethod === m ? 'bg-[#45055B] text-white border-[#45055B]' : 'bg-white text-[#45055B]/60 border-[#45055B]/20 hover:border-[#45055B]/40'
                }`}>{m.replace('_', ' ')}</button>
            ))}
          </div>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        {result ? (
          ...
          <div className="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            {/* Customer Name */}
            <div>
              <p className="text-[10px] font-bold text-[#45055B]/40 uppercase tracking-wider mb-2">Customer Name</p>
                ...
                <div className="space-y-2">
                  {shippingDisplay > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#45055B]/10 cursor-pointer">
                      ...
                      <span className="font-bold text-[#45055B]">${shippingDisplay.toFixed(2)}</span>
                    </label>
                  )}
                  {taxDisplay > 0 && (
                    <label className="flex items-center justify-between p-3 bg-[#FAF6F0] rounded-xl border border-[#45055B]/10 cursor-pointer">
                      ...
                      <span className="font-bold text-[#45055B]">${(isFullCancel ? taxDisplay : proratedTax).toFixed(2)}</span>
                    </label>
                  )}
                  {discountAmount > 0 && (
